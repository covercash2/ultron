...
const GNU_PLUS_LINUX: (&str, &str) = (
    "linux",
    r#"I'd just like to interject for a moment. What you're referring to as Linux, is in fact, GNU/Linux, or as I've recently taken to calling it, GNU plus Linux. Linux is not an operating system unto itself, but rather another free component of a fully functioning GNU system made useful by the GNU corelibs, shell utilities and vital system components comprising a full OS as defined by POSIX.
...
const GOOGLERS: (&str, &str) = (
    "googlers",
    r#"The key point here is our programmers are Googlers, they’re not researchers. They’re typically, fairly young, fresh out of school, probably learned Java, maybe learned C or C++, probably learned Python. They’re not capable of understanding a brilliant language but we want to use them to build good software. So, the language that we give them has to be easy for them to understand and easy to adopt."#,
);

const RUST: (&str, &str) = (
    "rust",
    "Rust has zero-cost abstractions, move semantics, guaranteed memory safety, threads without data races, trait-based generics, pattern matching, type inference, minimal runtime and efficient C bindings.",
);

const RICK_AND_MORTY: (&str, &str) = (
    "rick and morty",
...
pub const OPEN_SOURCE_MAINTAINERS: (&str, &str) = (
...
const COPY_PASTA_ARRAY: [(&str, &str); 6] = [
    GNU_PLUS_LINUX,
    GOOGLERS,
    RUST,
    RICK_AND_MORTY,
...
static COPY_PASTAS: OnceLock<HashMap<&str, &str>> = OnceLock::new();

fn init_map() -> HashMap<&'static str, &'static str> {
    COPY_PASTA_ARRAY.iter().cloned().collect()
}

/// get a list of all available copy pastas.
pub fn copy_pasta_names() -> Vec<&'static str> {
    COPY_PASTAS.get_or_init(init_map).keys().copied().collect()
}

/// get a copy pasta by its name.
pub fn copy_pasta(name: &str) -> Option<&str> {
    COPY_PASTAS.get_or_init(init_map).get(name).copied()
}
